...
import random
from textadv.gamesystem.parser import Matched

import os
GAME_LOCATION = os.path.dirname(os.path.realpath(__file__))

execfile(os.path.join(GAME_LOCATION, "config.py"))

execfile("textadv/basicsetup.py")

world[Global("release_number")] = "0.22 (April 7, 2013)"
...
if False :
    ...
    You've heard stories about tEp: the purple palace, a.k.a. the last
    stronghold of the knights of the lambda calculus.  You've been
    meaning to get over there to see what it's all about.
    
    [newline]But, you are lazy.  Instead, you went to the tEp website
    and opted for the <i>virtual</i> house tour.  We understand.
    ...
    world[Global("game_description")] = """
    Although you're still where you were, you're now where you are
    since [ob <Irving Q. Tep>] has brought you to the Purple Palace,
    253 Comm. Ave."""
...
def _str_eval_img(eval, act, ctxt, *obs) :
    ...
    try :
        ...
        #width, height = Image.open(os.path.join(GAME_LOCATION, "teptour_files", filename)).size
#        print "Image",filename,"is",width,"x",height
        #return ["<img class=\""+css_class+"\" width=\""+str(width)+"\" height=\""+str(height)+"\" src=\"teptour/"+filename+"\">"]

        img = "<img class=\""+css_class+"\" src=\"teptour/"+filename+"\">"

        head,tail = os.path.split(filename)
        raw = os.path.join(head, "raw", tail)
        if os.path.isfile(os.path.join(GAME_LOCATION, "teptour_files", raw)):
            img = "<a href=\"teptour/" + raw + "\" target=_blank>" + img + "</a>"
        return [img]
    except IOError :
        raise IOError("Missing image %r" % obs[0][0])
        return ["<i>(missing image %r)</i>" % obs[0][0]]
...
parser.understand("about", AskingAbout(actor, "Irving Q. Tep", "virtual house tour"))
parser.understand("quit", MakingMistake(actor, "{Bob} should try closing the tab instead."))

parser.understand("oh/oh./ok/ok.", MakingMistake(actor, "Exactly."))
parser.understand("cd . .", Exiting(actor)) # spaces between periods because parser treats periods as tokens
parser.understand("cd [somewhere x]", GoingTo(actor, X))
parser.understand("pwd/dir", Looking(actor))
parser.understand("where am i", Looking(actor))
parser.understand("where am i ?", Looking(actor))
parser.understand("cd", MakingMistake(actor, """But you already are home!"""))
parser.understand("honig", MakingMistake(actor, "\"Honig!\" you exclaim.  You can hear vigorous acclamation all around you."))
parser.understand("in the name of honig", MakingMistake(actor, "Amen."))
parser.understand("what ?", MakingMistake(actor, """You just had to
...
peldging, stop it!\" """))
parser.understand("i wanna peldge", MakingMistake(actor, """This is
virtual tEp.  If you have a bid and you're wanting to peldge, then say
the magic words where real tEp can hear you!"""))
parser.understand("pray/amen", MakingMistake(actor, """A chorus of
...
away."""))
parser.understand("go home", MakingMistake(actor, """But you already are!"""))
parser.understand("tep", MakingMistake(actor, """That's where you are."""))
parser.understand("why/how [text x]", MakingMistake(actor,
...
93]"""))
parser.understand("pickles ?", MakingMistake(actor,
"""\"I didn't make this tour!" [ob Irving] explains, "I just repeat what I'm told.\""""))

def make_face_action(face) :
    parser.understand(face, MakingMistake(actor, "Oooh! Let me try![newline]" + escape_str(face)))
map(make_face_action, [":)", ":-)", ":(", ":-(", ";-)", ":P", ":-P"])

parser.understand("gruesz [something x]", Taking(actor, X))

parser.understand("nice", MakingMistake(actor, """Thanks. Now don't forget to come by real-tEp&#8482;!"""))
parser.understand("nice tour", MakingMistake(actor, """Thanks. Now don't forget to come by real-tEp&#8482;!"""))

parser.understand("rm/su/sudo [text x]", MakingMistake(actor, "Nice try, buster."))

parser.understand("die", Attacking(actor, actor))

parser.understand("burn down tep", MakingMistake(actor, """The RA
comes out of nowhere, preventing {him}. "Remember the zeroth [ask
<rules of tep> <rule of tep>]!" he says, "Don't die!" """))
...
parser.understand("sleep", MakingMistake(actor, "You have no time for sleep when you have a house tour to do!"))

# the free willy net says you can't get out even though you can...
parser.understand("I thought I/you couldn't [text x]", MakingMistake(actor, "Sometimes we make mistakes."))
parser.understand("I thought you said I/you couldn't [text x]", MakingMistake(actor, "Sometimes we make mistakes."))
parser.understand("you said I/you couldn't [text x]", MakingMistake(actor, "Sometimes we make mistakes."))
parser.understand("but you said I/you couldn't [text x]", MakingMistake(actor, "Sometimes we make mistakes."))
...
quickdef(world, "rat", "backdrop", {
 	BackdropLocations : "everywhere",
        Words : ["@rat", "@rats"],
        Description : """It's a big, fat rat, scurrying around.""",
        NoTakeMessage : """You make a valiant attempt to get a hold of
        the rat, but it moves quickly.  Too quickly for you."""
        })

quickdef(world, "cat", "backdrop", {
 	BackdropLocations : "everywhere",
        Words : ["@cat", "@cats"],
        Description : """It's a cat!  It's also probably why there are
        so few rats around tEp these days.""",
        NoTakeMessage : """You manage to grab the cat for a while,
        petting it furiously and making various "it's a cat!" faces.
        But it ultimately worms its way out of your arms."""
        })

quickdef(world, "tEps", "person", {
        Words : ["tEp", "tEps"],
        Scenery : True,
        Description : """You don't see a tEp around here."""
        })

@before(Taking(actor, "tEps"))
def before_taking_tep(actor, ctxt) :
    raise AbortAction("""You don't see a tEp for you to take.""")

quickdef(world, "RA", "person", {
        Words : ["RA", "resident", "@advisor"],
        Scenery : True,
        Description : """Like Spiderman, the RA can feel when there's
        trouble, and he appears when he's needed.  And, that time is
        not now."""
        })

@before(Taking(actor, "RA"))
def before_taking_tep(actor, ctxt) :
    raise AbortAction("""Assuming you see him, the RA wouldn't appreciate that, which you don't.""")

@actoractivities.to("start_game")
@actoractivities.to("player_has_moved")
def put_tep_in_current_room(ctxt) :
    ctxt.world.activity.put_in("tEps", ctxt.world[VisibleContainer(ctxt.world[Location(ctxt.actor)])])
    ctxt.world.activity.put_in("RA", ctxt.world[VisibleContainer(ctxt.world[Location(ctxt.actor)])])
...
quickdef(world, "253 Commonwealth Ave", "room", {
        ...
        [ob <purple tree>] and a [ob <park bench>].

        [newline]To the [dir north] is the [ob door] to enter tEp.

        [newline]You can look [look east] and [look west] along the
...
world[DirectionDescription("253 Commonwealth Ave", "west")] = """
[img 1/253/look_w.JPG left]Toward the west, you see that the [ob
<purple tree>] is the only purple tree along the block."""
world[DirectionDescription("253 Commonwealth Ave", "east")] = """
[img 1/253/look_e.JPG left]Toward the east, you see that the [ob
<purple tree>] is the only purple tree along the block."""
world[DirectionDescription("253 Commonwealth Ave", "south")] = """
...
kind of monument."""
world[DirectionDescription("253 Commonwealth Ave", "up")] = """
[img 1/253/look_up.JPG left]You look up and see a bit of the [ob
<purple tree>] along with a few of the five floors of tEp."""

world[NoGoMessage("253 Commonwealth Ave", X)] = """Nah, you don't need
to leave that way!  This is a virtual tour: just close your web
browser if you want to quit."""
...
quickdef(world, "front garden", "container", {
        Scenery : True,
        IsEnterable : True,
        Description : """[img 1/253/garden.JPG left]This is the front
        garden, in which are the [ob <purple tree>] and a [ob <park
        bench>]."""
        },
...
quickdef(world, "purple tree", "thing", {
        ...
        avenue.  It's very purple.  Below it is the [ob <front
        garden>]."""
        },
         put_in="253 Commonwealth Ave")
...
quickdef(world, "park bench", "supporter", {
        Scenery : True,
        IsEnterable : True,
        Description : """[img 1/253/bench.jpg left]Sitting in the [ob
        <front garden>] is this handmade park bench wrought from
...
parser.understand("sit in [object front garden]", Entering(actor, "park bench"))
...
world.activity.connect_rooms("front door", "north", "The Foyer")

instead_of(actionsystem,
           Going(actor, "in") <= PEquals(ContainingRoom(actor), "253 Commonwealth Ave"),
           Going(actor, "north"), suppress_message=True)

world[IsLocked("front door")] = False
world[IsLocked("front door") <= PEquals(ContainingRoom("player"), "253 Commonwealth Ave")] = True
...
parser.understand("knock", MakingMistake(actor, """You knock on the
front door, but no one hears you.  Perhaps you should ring the [ob
doorbell].""") <= PEquals("253 Commonwealth Ave", ContainingRoom(actor)))
...
quickdef(world, "car", "thing", {
        Scenery : True,
        Description : """This is one of those cars that the neighbors
        like to buy."""
        }, put_in="253 Commonwealth Ave")

quickdef(world, "front steps", "supporter", {
        AddedWords : ["@stoop"],
        IsEnterable : True,
        Scenery : True,
        Description : """The front steps are a nice place to sit on in
        nice weather.  They lead up to the [ob <front door>] and into
        tEp."""
        }, put_in="253 Commonwealth Ave")
...
quickdef(world, "The Center Room", "room", {
        ...
        composite photos from the past decade, and a [ob chandelier]
        that seems like it has seen better days.  Looking up, you can
        see the [ob <center stairwell>].

        [newline]You can go [dir south] to the front room, [dir north]
        to the dining room, [dir upstairs] to the second floor, [dir
        northeast] to the back stairwell, or [dir southeast] back to
        the foyer, and you can look [look north], [look south], [look
        east], [look west], and [look up]."""
        })
add_floor(world, "The Center Room", "carpet")

world[DirectionDescription("The Center Room", "north")] = """
[img 1/center/look_n.JPG left]You see the [ob <comfy couch>], the [ob
King], the [ob <foosball table>], and the [ob <bulletin board>].  You
can go [dir north] into the dining room and [dir northeast] into the
back stairwell."""
world[DirectionDescription("The Center Room", "east")] = """
[img 1/center/look_e.JPG left]You can barely make out a [ob <foosball
table>].  You can go [dir upstairs] to the second landing and [dir
southeast] into the foyer."""
world[DirectionDescription("The Center Room", "south")] = """
[img 1/center/look_s.JPG left]On the wall is a [ob <zombie protection
box>]. You can go [dir southeast] into the foyer and [dir south] into
the front room."""
world[DirectionDescription("The Center Room", "west")] = """
[img 1/center/look_w.JPG left]You can see the [ob <comfy couch>] and
the [ob mantle]."""
world[DirectionDescription("The Center Room", "up")] = """
...
quickdef(world, "foosball table", "container", {
        AddedWords : ["foos", "fooz"],
        Scenery : True,
        Openable : True,
        ...
        much the foosball table cost us when we got it?"[if [get
        IsOpen <foosball table>]][newline]It is open, and in it you
        see [serial_comma [get Contents <foosball table>]].[endif]"""
        }, put_in="The Center Room")
parser.understand("play [object foosball table]", Using(actor, "foosball table"))
reset_action_handler(before, Using(actor, "foosball table"))
@report(Using(actor, "foosball table"))
def report_playing_foos(actor, ctxt) :
    ctxt.write(""" "Click! Click!" goes a volley before the ball skids
...
quickdef(world, "human skull", "thing", {
...
quickdef(world, "king", "thing", {
        PrintedName : "The King",
        ProperNamed : True,
        Scenery : True,
        NoTakeMessage : """That's always been there.  You shouldn't
        move it.""",
...
quickdef(world, "mantle", "supporter", {
        Scenery : True,
...
quickdef(world, "zombie protection box", "thing", {
        Scenery : True,
...
quickdef(world, "chandelier", "thing", {
        ...
        rousing game, all of the sconces exploded in a shower of
        glass.  It was really a sight to see."""
        },
         put_in="The Center Room")
...
@when(Jumping(actor) <= PEquals("ex_ball", Location(actor)), wants_event=True)
def when_jumping_on_ball(action, actor, ctxt) :
    ctxt.world.activity.put_in(actor, ctxt.world[ParentEnterable("ex_ball")])
    action.jump_handled = True
@report(Jumping(actor), wants_event=True)
def report_jumping_on_ball(action, actor, ctxt) :
    if hasattr(action, "jump_handled") :
        ctxt.write("You fly through the air with the greatest of whee's before falling to the ground.")
        raise ActionHandled()
...
world.activity.put_in("51", "r_center_stairs")

quickdef(world, "center stairwell", "backdrop", {
# Description overridden to looking up and down                
...
quickdef(world, "The Front Room", "room", {
        AddedWords : ["@fridge"],
        Visited : True,
        Description : """[img 1/front/look.JPG left]This is where the
        ...
        declared the purple paint to be "historic." There is a [ob
        piano], some [ob boats], and a [ob <rideable dinosaur>]. You
        can go [dir north] to the center room, and look [look south]."""
        })
add_floor(world, "The Front Room", "carpet")
world[DirectionDescription("The Front Room", "south")] = """
...
quickdef(world, "Super Smash Bros.", "thing", {
        AddedWords : ["@bros", "bros", "bros.", "@cartridge", "@smash"],
        Scenery : True,
        NoTakeMessage : """That would be cruel to take the house's
...
parser.understand("play [object Super Smash Bros.]", Using(actor, "Super Smash Bros."))
reset_action_handler(before, Using(actor, "Super Smash Bros."))
@report(Using(actor, "Super Smash Bros."))
def report_using_piano(actor, ctxt) :
    ctxt.write("""You turn on the N64 console, and a couple of tEps
    come to join you to play Super Smash Bros.  They beat you with
    their incredibly high-speed reflexes.  Maybe you'll do better next
    time.""")

quickdef(world, "vlca", "thing", {
        PrintedName : "The VLCA",
        ProperNamed : True,
        AddedWords : ["very", "large", "capacitor", "@array"],
        ...
        another pipe with an empty soda can on the end is placed
        across them, it emits bright sparks and a popping sound as the
        capacitors discharge."""
        }, put_in="The Front Room")

quickdef(world, "rideable dinosaur", "supporter", {
        IsEnterable : True,
        Scenery : True,
        NoTakeMessage : """There's too much stuff around it right now
        to dig it out.  Besides, it's kind of heavy.""",
        Description : """[img 1/front/dinosaur.JPG left]This is a
        rideable dinosaur.  It has wheels on the bottom for you to
        scoot around."""
        }, put_in="The Front Room")

class Riding(BasicAction) :
    """Riding(actor, x) for the actor riding a thing x."""
    verb = "ride"
    gerund = "riding"
    numargs = 2
parser.understand("ride/scoot [something x]", Riding(actor, X))
parser.understand("ride/scoot/charge", Riding(actor, "rideable dinosaur") <= PEquals("rideable dinosaur", Location(actor)))
parser.understand("ride/scoot/charge !", Riding(actor, "rideable dinosaur") <= PEquals("rideable dinosaur", Location(actor)))

require_xobj_accessible(actionsystem, Riding(actor, X))
@trybefore(Riding(actor, X))
def trybefore_riding_enter_x(actor, x, ctxt) :
    if x != ctxt.world[Location(actor)] :
        ctxt.actionsystem.do_first(Entering(actor, x), ctxt, silently=True)
    if x != ctxt.world[Location(actor)] :
        raise AbortAction(str_with_objs("{Bob} needs to be on [the $x] to ride [him $x].", x=x), actor=actor)
@before(Riding(actor, X))
def before_riding_cant(actor, x, ctxt) :
    raise AbortAction("{Bob} can't ride that.", actor=actor)

reset_action_handler(before, Riding(actor, "rideable dinosaur"))
@report(Riding(actor, "rideable dinosaur"))
def report_riding_dinosaur(actor, ctxt) :
    ctxt.write("""{Bob} {takes} the rideable dinosaur out for a spin.
    The wind blows through {his}
    hair and {he} {feels} alive.  With {his} spirit rejuvenated, {he}
    {parks} the dinosaur back where {he} found it.""", actor=actor)


quickdef(world, "boats", "thing", {
        Words : ["kayak", "kayaks", "boat", "boats", "canoe", "canoes"],
        Scenery : True,
        NoTakeMessage : """The boat is attached to the ceiling.""",
        Description : """[img 1/front/kayak.JPG left]We have a kayak
        and a canoe that people take out and ride on the Charles
        River."""
        }, put_in="The Front Room")
quickdef(world, "piano", "thing", {
        Scenery : True,
        Description : """[img 1/front/piano.JPG left]It's a really
        out-of-tune piano, but surprisingly in tune considering how it
        hasn't been tuned in recent memory.  It's still used
        frequently, however.  Sitting on the piano is the score for
        Beethoven's Pathetique sonata."""
        }, put_in="The Front Room")

parser.understand("play [object piano]", Using(actor, "piano"))
reset_action_handler(before, Using(actor, "piano"))
@report(Using(actor, "piano"))
def report_using_piano(actor, ctxt) :
    ctxt.write("""Despite the lack of intonation, {bob} {plays} a
    beautiful melody on the piano.  {He} didn't know {he} had it in
    {him}.""", actor=actor)
